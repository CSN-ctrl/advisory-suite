...
import { createDefaultDocument, normalizeCanvasDocument, type CanvasDocument } from "@/lib/canvas-document";
...
function mapRow(row: Record<string, unknown>): SitePageRow {
  return {
    ...
    document: normalizeCanvasDocument(row.blocks),
...
export async function insertSitePage(input: {
  ...
  const { data, error } = await sb
    ...
    .insert({
      ...
      sort_order: 0,
      published: true,
      blocks: input.document ?? createDefaultDocument(),
